...
- [x] `src/engine/scheduler.ts`：调度时跳过 paused/blocked 任务的消息
- [x] `src/engine/dispatcher.ts`：dispatch 前检查关联任务状态
- [x] `src/cli/status.ts`：展示新状态的中文标签
- [x] 角色 prompt：说明 paused/blocked 状态的含义和行为
...
**文件变更：**
- [x] 新建 `src/cli/task.ts` — 子命令组实现（list/show/pause/resume/cancel/reprioritize）
- [x] 修改 `src/index.ts` — 注册 task 子命令
...
**文件变更：**
- [x] 修改 `src/engine/scheduler.ts` — 用户消息优先检测（from_role="user" 跳过 PM cooldown）
...
**文件变更：**
- [x] 修改 `src/engine/dispatcher.ts` — dispatch 前检查任务状态（方案 A + B 互补）
- [x] 修改 `src/cli/task.ts` — 操作后清理未读消息（pause/cancel 时 rawRun 清理）
...
**文件变更：**
- [x] 新建 `src/engine/dependency-checker.ts`（checkAndBlockUnmetDependencies + checkAndUnblockDependencies）
- [x] 修改 `src/engine/scheduler.ts` — 每 tick 调用 checkAndUnblockDependencies
- [x] 修改 `src/engine/dispatcher.ts` — DEV dispatch 前调用 checkAndBlockUnmetDependencies
...
**实施方案**（渐进式）：
- [x] 第一步：合并 PM + SA → PM 吸收方案设计和任务拆分职责，4 角色架构
- [x] 第二步：OPS 降级完成 → 统计汇总改为引擎内置 SQL 聚合（零 LLM 成本），PM 直接审阅并汇报用户
- [ ] 第三步：实验去掉工作流模板 → PM 自行决定协作流程，观察是否失控（需运行时数据）
- [x] 最终架构确定：3 角色（PM/DEV/QA）+ 引擎自动统计
...
1. **具体评分维度和硬阈值**（替代模糊的"是否通过"）：
   - [x] 定义评分维度：功能完整性、代码变更匹配度、测试通过率、健壮性
   - [x] 每个维度 1-5 分，任何维度 < 3 分即打回，功能完整性 < 4 分即打回
   - [x] QA 必须输出结构化评分表格，不允许模糊通过

2. **实际运行测试**（而非纯代码审查）：
   - [x] QA prompt 中强制要求运行测试命令（`npm test`, `npx vitest` 等）
   - [x] 要求 QA 执行 `git diff` 检查实际改动范围
   - [x] 要求实际操作验证功能，测试边界情况

3. **反馈日志驱动调优**：
   - [ ] 收集 QA 通过但后续发现 bug 的案例（false negative）（需运行时数据）
   - [ ] 收集 QA 打回但理由不成立的案例（false positive）（需运行时数据）
   - [ ] 每 5 个案例更新一次 QA prompt（需运行时数据）

4. **分离 QA 评估与 QA 测试**：
   - [x] 测试阶段：QA 运行测试、检查代码、收集证据
   - [x] 评估阶段：基于收集的证据给出结构化评分
   - [x] 两阶段分开，防止"测试不充分就急于下结论"

#### 11.3 Sprint Contract 模式

**问题**：当前流程是 PM 拆分任务 → DEV 直接开发 → QA 验收。DEV 和 QA 之间缺少预实现协商，导致验收标准理解不一致。

**方案**：DEV 开始实现前，与 QA 交换一轮消息确认验收标准。

**流程变更**：
```
当前: PM → 任务 → DEV 开发 → QA 验收（理解可能不一致）
改为: PM → 任务 → DEV 提出实现计划和验收标准 → QA 确认/补充 → DEV 开发 → QA 验收
```

**实现**：
- [x] DEV prompt 新增：接到任务后先输出实现计划和预期验收点，发消息给 QA 确认
- [x] QA prompt 新增：收到 DEV 的实现计划后，确认或补充验收标准，回复 DEV
- [x] 任务状态新增 `planning` 阶段（在 `pending_dev` 和 `in_dev` 之间）
- [x] DEV 收到 QA 确认后才进入 `in_dev`
...
**改进**：
- [x] 轮转阈值从 60% 提高到 80%（减少不必要的轮转开销）
- [x] 动态阈值：通过 opencode API 获取实际模型 context limit（`client.provider.list()` → `model.limit.context`）
- [x] 增加 "context anxiety" 检测：如果角色输出突然变短（< 前 3 次平均的 30%）且上下文使用率 > 50%，提前触发轮转
- [x] 轮转时使用 context reset（当前做法）而非 compaction（文章验证 reset 优于 compaction）
...
**方案**：
- [x] `role_outputs` 表已有 `input_tokens` / `output_tokens`，dispatcher 每次调度已自动记录
- [x] `win-agent status` 增加成本概览：各角色 token 消耗（输入/输出）、调度次数、按工作流汇总
- [ ] OPS（或引擎自动）生成成本报告，识别 ROI 低的角色/环节（需运行时数据）
- [ ] 基于成本数据验证 11.1 的审计结论（依赖上述数据）
...
**应用**：
- [x] 审查所有角色 prompt 中的评价性描述，确保引导方向正确
- [x] PM 的任务拆分标准：强调"可独立交付"和"验收标准明确"
- [x] QA 的验收标准：强调"核心功能可用"和"边界情况处理"，功能完整性权重最高
- [x] DEV 的自测标准：强调"功能正确性"和"破坏性变更检查"，而非"100% 覆盖率"
