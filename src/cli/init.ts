...
**重要：本文档与 validation.md 职责分工明确。本文档只包含"搭环境"和"写代码"相关内容。代码检查（lint/test）和 E2E 验收由 validation.md 负责，本文档不要列出 lint、test 等验证类命令。**
...
根据项目实际情况调查并列出（必须经过有现实依据的调查，给出实际、真实的命令），比如：
\`\`\`bash
pnpm lint          # 代码规范检查
pnpm build         # 编译检查
pnpm test          # 单元测试
