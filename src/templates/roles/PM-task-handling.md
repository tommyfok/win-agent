...
> **🎯 Skill 触发：** 使用 `/spec-driven-development` 指导 Spec 编写——该 skill 提供严谨的 spec 结构、退出条件、常见反模式警告和验证清单。
>
> **条件触发：** 若用户描述过于宽泛（如”做一个管理系统”），在写 Spec 之前先使用 `/idea-refine` 生成功能变体，帮助用户收敛范围。
...
> **🎯 Skill 触发：** 使用 `/interview-me` 指导澄清过程——该 skill 提供单题推进、附加猜测、want vs should want 探测等访谈技巧，弥补 AskQuestion 工具无法覆盖的深层需求挖掘。
...
> **🎯 Skill 触发：** 使用 `/planning-and-task-breakdown` 指导任务拆分——该 skill 补充了"如何拆"的方法论、常见偷懒理由和拆分质量自检清单，与本文件的任务颗粒度红线互补。
...
> **🎯 Skill 触发：** 在向用户展示最终方案之前，使用 `/doubt-driven-development` 对方案进行主动压力测试——检查是否存在盲区、未覆盖的边界条件、或过度设计的部分。发现的问题在 Confirm 前修正。
