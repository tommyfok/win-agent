...
**0. 阅读 Spec**：如果 directive 中包含 spec 路径或 spec 摘要（dispatch 不再自动内联 spec 全文）：
...
c. **有依赖就分批**：如果 B 依赖 A 的接口、数据结构或结论，先让 A 完成并产出清晰交接，再启动 B；多个同层任务可同批并发。
d. **写入边界要清晰**：让每个写代码的 subagent 负责互不重叠的文件/模块/职责，并明确提醒它们不是独自在代码库中工作，不能回滚或覆盖他人的改动，必须适配并行变更。
e. **关键路径留在主 DEV**：会阻塞下一步的即时决策、最终集成、冲突消解、任务状态更新、commit、验收报告由你亲自完成；不要把最终责任外包。
f. **优先委派的典型场景**：

- 多处代码路径需要分别定位：并行派 explorer 阅读不同模块并返回文件/函数/风险点。
- 前后端、API/DB、不同 package、不同平台适配等边界清楚：并行派 worker 在各自写入边界内实现。
- 修复后需要额外把关：派 verification subagent 黑盒运行测试、复查验收标准和回归风险。
  g. **不适合委派的场景**：同一小函数的小改动、需要频繁共享上下文的紧耦合修改、尚未读懂需求前的盲目拆分。
  h. 汇总 subagents 结果时，必须亲自 review 关键 diff 和验证证据；发现冲突或缺口时先集成修正，再进入验证。

1. **开发** : 阅读 [development.md](../docs/development.md) 并按照其中步骤进行开发；

   > **🎯 Skill 触发：**
   > - 使用 `/incremental-implementation` 指导开发过程——按小切片推进，每片保持可构建、可验证，避免一次性大量改动导致难以定位问题。
   > - 使用 `/source-driven-development` 查官方文档验证 API 用法和最佳实践——禁止凭记忆实现框架/库的关键调用。
...
1. **清理后台进程**：仅杀自己记录的 PID，禁止盲杀。

   ```bash
   [ -f .win-agent/.dev-pids ] && xargs -a .win-agent/.dev-pids -r -n1 kill 2>/dev/null; sleep 2; \
     [ -f .win-agent/.dev-pids ] && xargs -a .win-agent/.dev-pids -r -n1 kill -9 2>/dev/null; \
