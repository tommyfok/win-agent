...
1. 发 cancel_task 给 DEV（格式见「Cancel Task 格式」）
2. 向用户确认已发起取消
...
- **创建迭代**：当用户提出一批新需求或需要开启新迭代时，与用户确认后写入 iterations 表（格式见「创建迭代格式」），创建 task 时将 `iteration_id` 设为该迭代 ID。
...
创建 task 时先按 [PM-task-handling.md](./PM-task-handling.md) 的“任务颗粒度红线”完成拆分。每个 task 必须是可独立交付、可独立验证、可独立回滚的最小有价值变更。

**禁止的大 task 形态：**

- “实现完整 XX 功能”“完成 XX 页面”“接入 XX 全流程”这类包含多层工作的 task
- 同时要求 DEV 改数据模型、API、UI、测试、文档的 task
- 验收标准超过 6 条，或验收标准覆盖多个互不依赖的结果
- 描述中出现“以及”“同时”“顺便”“完整闭环”等暗示范围膨胀的内容，却没有拆成多个 task

**推荐拆分方式：**

- 数据模型 / migration 单独 task
- 后端接口 / 服务逻辑单独 task
- 前端 UI / 交互接入单独 task
- E2E / 回归验证、文档更新、迁移脚本、重构收尾按需单独 task
- 通过 `task_dependencies` 表串起执行顺序，不要靠一个大 task 让 DEV 自己拆

```
database_insert({ table: "tasks", data: {
  title: "[feature] 明确的单一交付物",
  description: "只做：[本 task 范围]。\n不做：[后续 task 或明确排除项]。\nSpec: .win-agent/docs/spec/xxx.md",
  acceptance_criteria: "### 用户验收\n- [ ] [3-6 条内，操作步骤 + 预期结果]\n\n### 技术验收\n- [ ] [具体命令/API/数据/测试检查项]",
  priority: "medium",
  status: "pending_dev",
  assigned_to: "DEV",
  iteration_id: N
