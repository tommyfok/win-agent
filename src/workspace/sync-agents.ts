...
// Open database connection (lazy singleton)
let _db: any = null;
function getDb(directory: string) {
  if (!_db) {
    const dbPath = path.join(directory, "${dbRelPath}");
    _db = new Database(dbPath);
    _db.pragma("journal_mode = WAL");
    _db.pragma("foreign_keys = ON");
    try { sqliteVec.load(_db); } catch {}
  }
  return _db;
}
...
export const query = {
  ...
  args: {
    ...
    where: z.record(z.any()).optional().describe("查询条件（键值对）"),
...
export const insert = {
  ...
  args: {
    table: z.enum(TABLES).describe("表名"),
    data: z.record(z.any()).describe("要插入的数据"),
  },
  async execute(args: any, ctx: any) {
    const db = getDb(ctx.directory);
    ...
    const values = keys.map((k: string) => args.data[k]);
...
export const update = {
  ...
  args: {
    ...
    data: z.record(z.any()).describe("要更新的字段"),
  },
  async execute(args: any, ctx: any) {
    const db = getDb(ctx.directory);
    ...
    const params: any[] = [];
    const setClauses = Object.keys(args.data).map((key: string) => {
      params.push(args.data[key]);
      return \`\${key} = ?\`;
    });
